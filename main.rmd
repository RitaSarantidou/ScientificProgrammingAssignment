...
```{r}
ddf <- knnImputation(df, k = 5, scale = T)  # check about the scalling again 
anyNA(ddf)
ddf <- ddf[,5:47287]
...
balance_bipolar <- ROSE(condition~., data = testset[1:31,], N = 60)
table(balance_bipolar$data$condition)
bp <-balance_bipolar$data
nm <- subset(bp, condition =="normal")
bpd <- subset(bp, condition == "bipolar")

balance_scz <- ROSE(condition~., data = testset[17:45,], N = 60)
table(balance_scz$data$condition)
sc <-balance_scz$data
...
nm3 <- sample_n(nm, 15)  # randomly select 150 rows
nm4 <- sample_n(nm2, 15)

NM <- rbind(nm3, nm4)  # bind the normal subsets

testset_ROSE <- rbind(bpd,NM, sch)  # Bind the bipolar, normal, and schizophrenic subsets
table(testset_ROSE$condition)
#str(df_red)
# testset_ROSE$condition<- as.character(testset_ROSE$condition)
# 
# testset_ROSE$condition[testset_ROSE$condition == 0]<-"bipolar"
# testset_ROSE$condition[testset_ROSE$condition == 1]<-"normal"
# testset_ROSE$condition[testset_ROSE$condition == 2]<-"schizophrenia"
# table(testset_ROSE$condition)
# 
# testset_ROSE$condition <- as.factor(testset_ROSE$condition)

```
```{r}
 set.seed(77)
 ada <- as.data.frame(testset)
 ada$condition <- as.factor(ada$condition)
 testset_ADA <- AdasynClassif(condition~., ada, beta = 1, baseClass = "normal")
 table(testset_ADA$condition)
 
# testset_ADA$condition <- as.character(testset_ADA$condition)
# 
# testset_ADA$condition[testset_ADA$condition == 0]<-"bipolar"
# testset_ADA$condition[testset_ADA$condition == 1]<-"normal"
# testset_ADA$condition[testset_ADA$condition == 2]<-"schizophrenia"
# 
# testset_ADA$condition <- as.factor(testset_ADA$condition)
# table(testset_ADA$condition)
```
...
Plot SVM 
```{r}
plot(svm_ROSE, trainset, PC1 ~ PC3)
```
